...
async def run_competitor_scan(client: httpx.AsyncClient, settings: dict, guard: CostGuard | None = None, ai_client: OpenAIClient | None = None, do_ai: bool = True, state: StateStore | None = None, seen: set | None = None, weekly: bool = True) -> dict:
    ...
    logger.info(
        "[competitor pipeline] collected=%d after_window=%d removed_by_window=%d after_seen=%d removed_by_seen=%d after_filter=%d dedup_removed=%d high_signal=%d llm_input=%d report_events=%d empty_reason=%s",
